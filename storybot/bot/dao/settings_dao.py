...
_settings_index_created = False


def _get_collection() -> AsyncIOMotorCollection:
    """Return the settings collection inside the DB named in the DSN."""
    return _get_client().get_default_database().settings


async def _ensure_indexes() -> None:
    global _settings_index_created
    if not _settings_index_created:
        await _get_collection().create_index("target_username", sparse=True)
        _settings_index_created = True
